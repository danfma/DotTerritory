...
public static partial class Territory
{
    ...
    public static bool BooleanPointOnLine(Point point, LineString line, bool ignoreEndVertices = false, double? epsilon = null)
    {
        if (point == null)
        ...
        for (int i = 0; i < lineCoords.Length - 1; i++)
        {
            ...
            if (IsPointOnLineSegment(lineCoords[i], lineCoords[i + 1], pointCoord, ignoreBoundary, epsilon))
            {
                return true;
            }
        }
    ...
    /// <param name="epsilon">Maximum tolerance distance for a point to be considered on a line segment</param>
    /// <returns>True if the point is on the line</returns>
    public static bool BooleanPointOnLine(IFeature pointFeature, LineString line, bool ignoreEndVertices = false, double? epsilon = null)
    {
    ...
    /// <param name="epsilon">Maximum tolerance distance for a point to be considered on a line segment</param>
    /// <returns>True if the point is on the line</returns>
    public static bool BooleanPointOnLine(Point point, IFeature lineFeature, bool ignoreEndVertices = false, double? epsilon = null)
    {
        if (lineFeature == null)
        ...
        if (!(lineFeature.Geometry is LineString line))
            throw new ArgumentException("Feature geometry must be a LineString", nameof(lineFeature));
    ...
    /// <param name="epsilon">Maximum tolerance distance for a point to be considered on a line segment</param>
    /// <returns>True if the point is on the line</returns>
    public static bool BooleanPointOnLine(IFeature pointFeature, IFeature lineFeature, bool ignoreEndVertices = false, double? epsilon = null)
    {
        if (pointFeature == null)
            throw new ArgumentNullException(nameof(pointFeature), "Point feature is required");
    ...
    private static bool IsPointOnLineSegment(Coordinate lineSegmentStart, Coordinate lineSegmentEnd, Coordinate pt, string? excludeBoundary, double? epsilon)
    {
        double x = pt.X;
        double y = pt.Y;
