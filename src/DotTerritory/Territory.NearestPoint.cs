...
public static partial class Territory
{
    ...
    public static (Point Point, Length Distance, int Index) NearestPoint(Point targetPoint, params Point[] points)
    {
        if (targetPoint == null)
    ...
    public static (IFeature Feature, Length Distance, int Index) NearestPoint(Point targetPoint, FeatureCollection pointFeatures)
    {
        if (targetPoint == null)
            throw new ArgumentNullException(nameof(targetPoint), "Target point is required");
            
        if (pointFeatures == null || pointFeatures.Count == 0)
            throw new ArgumentException("Feature collection is empty or null", nameof(pointFeatures));
        ...
        if (nearestFeature == null)
            throw new ArgumentException("No valid point features found in the collection", nameof(pointFeatures));
    ...
    public static (Point Point, Length Distance, int Index) NearestPoint(IFeature targetPointFeature, params Point[] points)
    {
        if (targetPointFeature == null)
            throw new ArgumentNullException(nameof(targetPointFeature), "Target point feature is required");
            
        if (!(targetPointFeature.Geometry is Point targetPoint))
            throw new ArgumentException("Target feature must have a Point geometry", nameof(targetPointFeature));
    ...
    /// <param name="pointFeatures">A feature collection of points to find the nearest one from</param>
    /// <returns>The closest point feature from the collection to the reference point, along with the distance and index</returns>
    public static (IFeature Feature, Length Distance, int Index) NearestPoint(IFeature targetPointFeature, FeatureCollection pointFeatures)
    {
        if (targetPointFeature == null)
            throw new ArgumentNullException(nameof(targetPointFeature), "Target point feature is required");
